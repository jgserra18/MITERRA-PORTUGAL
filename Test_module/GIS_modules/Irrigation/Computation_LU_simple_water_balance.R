...
for (i in year) {
  for (j in effs) {
    allocate_LU_gross <- allocate_LU_volume_GIS(year = i)# gross
    #allocate_LU_net <- allocate_LU_volume_GIS(year = i, irrig_eff = j) net
    map_LU_GIS(i, 'Volumes', 'missing', file_pattern = 'LU_vol', write = TRUE)
  }
}
map_LU_GIS(2009, 'Volumes', 'missing', file_pattern = 'LU_vol', write = TRUE)
